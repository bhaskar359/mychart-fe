...
export const ScheduleStepper: React.FC<ScheduleStepperProps> = ({ currentStep, steps }) => {
    return (
                    ...
                    <React.Fragment key={step.id}>
                        ...
                        <div className="flex flex-col items-start min-w-0">
                            <div className="flex items-center">
                                {/* Step Number Circle */}
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm mr-2 flex-shrink-0 ${numberClasses}`}>
                                    {step.id}
                                </div>
                                {/* Step Text */}
                                <div className={`flex flex-col text-left ${textClasses} text-sm whitespace-nowrap`}>
                                    {step.subtitle && <span className="text-xs text-muted-foreground">{step.subtitle}</span>}
